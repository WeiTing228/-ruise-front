...
export const checkout = {
                    ...
                    <form class="woocommerce-form-coupon">
                        <div class="form-group">
                            ...
                            <select class="name" v-model="order.coup_no">
                                <option v-for="(item, index) in coupon" :key="index" :value="item.coup_no">{{item.coupName}}</option>
                            </select>
                        </div>
                    </form>
                </div>
    ...
    async mounted() { 
        ...
        try {
            const response = await fetch(`http://localhost:8081/TIA103G3_Servlet/CouponsOwned?action=member&membId=${sessionStorage.getItem('id')}`, {
                method: 'GET',
                mode: 'cors',
            })
        if (response.status === 200) { 
